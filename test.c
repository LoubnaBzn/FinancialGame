...
int hello()
{
	...
	char *sqlSelection = "SELECT * FROM companies";
	...
	codeRetour = sqlite3_open("finance.db", &db);
	if (codeRetour){
	...
	*/
	codeRetour = sqlite3_exec(db, "CREATE TABLE companies(id INTEGER PRIMARY KEY AUTOINCREMENT, stock_name TEXT, closing_price TEXT, closing_date TEXT)", NULL, 0, &messageError);
	...
	/*
		...
		printf("Insertion des entreprises!\n");
		//on insère quelques valeurs dans la base de données
		sqlite3_exec(db, "INSERT INTO companies(stock_name) VALUES ('Apple')", NULL, 0, &messageError);
		if (codeRetour && messageError != NULL){
			sqlite3_free(messageError);
			messageError = NULL;
		}
		...
		sqlite3_exec(db, "INSERT INTO companies(stock_name) VALUES ('Google')", NULL, 0, &messageError);
		if (codeRetour && messageError != NULL){
			sqlite3_free(messageError);
			messageError = NULL;
		}
...
//http://www.supinfo.com/articles/single/1162-utilisation-sqlite-c-c
